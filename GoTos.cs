...
namespace WFA_COMPILADORES_AJ
{
    class Gotos
    {
        public static List<DatosTabla1> kernel = new List<DatosTabla1>();
        ...
        public void CalcularQuerners()
        {
            for (int q = 0; q < kernel.Count; q++)
            {
                //Generar Estados
                JuntarListas(q);
                for (int n = 0; n < Produccion.Count; n++)
                {
                    ...
                    if (PosPunto + 1 < Produccion[n].Count)
                    {
                        ...
                        {
                            DatosTabla1 NuevoEstado = new DatosTabla1();
                            NuevoEstado.N = kernel.Count;
                            ...
                            kernel.Add(NuevoEstado);
                        }
                        else //Ya existe
                        {
                            //Actualizar Goto
                            kernel[Posicion].GE.Add(q);
                            if (E < 0)
                            {
                                kernel[Posicion].GT.Add(ObtenerNoTerminal(E));
                            }
                            else
                            {
                                kernel[Posicion].GT.Add(Gramatica.ListaTokens[E - 1].simbolo);
                            }
        ...
        public void Generar_sub_estados(int estado, List<List<int>> LA, List<string> NT, List<List<int>> Nucleo)
        {
            ...
            for (int i = 0; i < kernel[estado].PN.Count; i++)
            {
                for (int j = 0; j < NT.Count; j++)
                {
                    if (kernel[estado].NTN[i] == NT[j] && NucleoIgual(kernel[estado].PN[i], Nucleo[j]))
                    {
                        if (AgregarElementos(kernel[estado].LAN[i], LA[j]))
                        {
        ...
        public void InicializarCambios(int estado, List<List<bool>> Cambios)
        {
            ...
            for (int i = 0; i < kernel[estado].P.Count; i++)
            {
                Cambios[1].Add(false);
            }
        }
        ...
        public void ActualizarEstados(int estado, List<List<bool>> Cambios)
        {
            for (int i = 0; i < Cambios[0].Count; i++)
            {
                ...
                if (Cambios[0][i])
                {
                    int X = kernel[estado].PN[i].IndexOf(Punto) + 1;
                    if (X < kernel[estado].PN[i].Count)
                    {
                        int GNT = kernel[estado].PN[i][X];
                        int SigEstado;
                        if (GNT < 0)
                        {
                        ...
                        if (SigEstado != -1)
                        {
                            Generar_sub_estados(SigEstado, kernel[estado].LAN, kernel[estado].NTN, kernel[estado].PN);
                        }
                    }
                }
            }
            for (int i = 0; i < Cambios[1].Count; i++)
            {
                if (Cambios[1][i])
                {
                    int X = kernel[estado].P[i].IndexOf(Punto) + 1;
                    if (X < kernel[estado].P[i].Count)
                    {
                        int GNT = kernel[estado].P[i][X];
                        int SigEstado;
                        if (GNT < 0)
                        {
                            SigEstado = EncontrarEstado(ObtenerNoTerminal(GNT), estado);
                        ...
                        if (SigEstado != -1)
                        {
                            Generar_sub_estados(SigEstado, kernel[estado].LA, kernel[estado].NT, kernel[estado].P);
                        }
                    }
                }
            }
        ...
        public int EncontrarEstado(string GNT, int GE)
        {
            ...
            {
                for (int j = 0; j < kernel[i].GT.Count; j++)
                {
                    if (kernel[i].GT[j] == GNT)
                    {
                        if (kernel[i].GE[j] == GE)
                        {
                            return i;
        ...
        {
            int PosPunto = kernel[estado].PN[PosNucleo].IndexOf(Punto);
            if (PosPunto + 1 < kernel[estado].PN[PosNucleo].Count)
            {
                int E = kernel[estado].PN[PosNucleo][PosPunto + 1];
                if (E < 0)
                {
                    bool produccion = false;
                    List<int> LA = new List<int>();
                    for (int i = PosPunto + 2; i < kernel[estado].PN[PosNucleo].Count; i++)
                    {
                        int EA = kernel[estado].PN[PosNucleo][i];
                        if (EA > 0)
                        {
                            if (!LA.Contains(EA))
                            {
                    ...
                    if (!produccion)
                    {
                        AgregarElementos(LA, kernel[estado].LAN[PosNucleo]);
                        for (int u = 0; u < kernel[estado].LA.Count; u++)
                        {
                            if (kernel[estado].NT[u] == ObtenerNoTerminal(E))
                            {
                                if (AgregarElementos(kernel[estado].LA[u], kernel[estado].LAN[PosNucleo]))
                                {
                                    Cambios[1][u] = true;
                                    ActualizarLAP(kernel[estado], u, kernel[estado].NT[u], LA);
                                }
                            }
                        }
                    }
        ...
        public bool ExisteEstado(List<List<int>> Nucleo, List<string> NT)
        {
            ...
            for (int i = 0; i < kernel.Count; i++)
            {
                if (Nucleo.Count == kernel[i].NTN.Count)
                {
                    for (int j = 0; j < Nucleo.Count; j++)
                    {
                        for (int u = 0; u < kernel[i].NTN.Count; u++)
                        {
                            if (NT[j] == kernel[i].NTN[u] && EstadoIgual(Nucleo[j], kernel[i].PN[u]))
                            {
                                encontro++;
                            }
                        }
        ...
        public void JuntarListas(int estado)
        {
            ...
            for (int i = 0; i < kernel[estado].PN.Count; i++)
            {
                Produccion.Add(kernel[estado].PN[i]);
                LookAhead.Add(kernel[estado].LAN[i]);
                NoTerminales.Add(kernel[estado].NTN[i]);
            }
            for (int i = 0; i < kernel[estado].P.Count; i++)
            {
                Produccion.Add(kernel[estado].P[i]);
                LookAhead.Add(kernel[estado].LA[i]);
                NoTerminales.Add(kernel[estado].NT[i]);
            }
        }

        public void GenerarEstado(string NoTerminal, List<int> LA)
        {
            DatosTabla1 nuevo = new DatosTabla1();
            nuevo.N = kernel.Count;
        ...
        public void ActualizarLAP(DatosTabla1 nuevo, int pos, string nt, List<int> LA)
        {
        ...
        public void GenerarProduccion(string NoTerminal, DatosTabla1 nuevo, List<int> LA)
        {
            int nuevos = 0;
        ...
        public bool ProduccionExiste(List<int> produccion, DatosTabla1 estado, string nt)
        {
            EsNucleo = false;
